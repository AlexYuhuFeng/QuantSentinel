...
def test_workspace_shell_template_snapshot() -> None:
    ...
    assert 'data-testid="workspace-main"' in layout_text
    assert 'data-testid="workspace-drawer"' in layout_text
    assert "def render_workspace_shell(" in layout_text


def test_each_workspace_has_toolbar_main_and_drawer_sections() -> None:
    for page in WORKSPACE_PAGES:
        ...
        assert "_render_main" in content, f"{page} must define main section"
